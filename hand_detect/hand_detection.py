...
def detect_face(frame, block=False, colour=(0, 0, 0)):
    ...
    for (x, y, w, h) in faces:
        if w * h > area:
            area = w * h
            X, Y, W, H = x, y, w, h
    cv2.rectangle(frame, (X, Y), (X + W, Y + H+75), colour, fill)
    #return X, Y, W, H
...
def capture_histogram(source=0):
    ...
    while True:
        ...
        cv2.rectangle(frame, (500, 300), (580, 380), (105, 105, 105), 2)
        box = frame[305:375, 505:575]
...
def locate_object(frame, object_hist):
    ...
    _, segment_thresh = cv2.threshold(object_segment, 150, 255, cv2.THRESH_BINARY)
...
def detect_hand(original, frame, hist):
    detected_hand, masked, raw = locate_object(frame, hist)
    return Hand(detected_hand, masked, raw, frame, original)